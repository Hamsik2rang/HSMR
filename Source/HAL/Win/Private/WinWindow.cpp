...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_ACTIVATE:
	{
		PushNativeEvent(s_boundHsWindow, HS::EWindowEvent::OPEN);
	...
	case WM_SIZE:
	{
		...
		if (isMinimized)
		{
			PushNativeEvent(s_boundHsWindow, HS::EWindowEvent::MINIMIZE);
		}
		else if (isMaximized)
		{
			PushNativeEvent(s_boundHsWindow, HS::EWindowEvent::MAXIMIZE);
		}
		else
		{
			if (width != s_boundHsWindow->surfaceWidth || height != s_boundHsWindow->surfaceHeight)
			{
				PushNativeEvent(s_boundHsWindow, HS::EWindowEvent::RESIZE);
			}
		}
	...
	{
		PushNativeEvent(s_boundHsWindow, HS::EWindowEvent::MOVE_ENTER);

		break;
	}
	case WM_EXITSIZEMOVE:
	{
		PushNativeEvent(s_boundHsWindow, HS::EWindowEvent::MOVE_EXIT);
		break;
	}
	case WM_MOVE:
	{
	...
	case WM_CLOSE:
	{
		PushNativeEvent(s_boundHsWindow, HS::EWindowEvent::CLOSE);
		PostQuitMessage(0);
		break;
	}
	default:
