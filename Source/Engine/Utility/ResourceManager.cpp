...
bool ResourceManager::Initialize()
{
	...
	if (context)
	{
		s_resourcePath = context->resourceDirectory;
	...
	HS_LOG(info, "ResourceManager initialized with path: %s", s_resourcePath.c_str());
...
const Image& ResourceManager::GetFallbackImage2DWhite()
{
	if (!s_isInitialize)
	{
		HS_LOG(crash, "ResourceManager is not initialized. Cannot get fallback image.");
	}
...
const Mesh& ResourceManager::GetFallbackMeshPlane()
{
	if (!s_isInitialize)
	{
		HS_LOG(crash, "ResourceManager is not initialized. Cannot get fallback mesh.");
	}
